...
### 改动
- 「复制全部」按钮改为「分享日志」，点击后将日志写入缓存文件并通过 FileProvider 分享给其他 App（微信、Telegram 等）
- 新增 FileProvider 配置
...
### Bug 修复
- 点击"启动服务"后，系统可能延迟或不绑定 NotificationListener，导致日志为空、不收集通知
- 修复：`ForegroundService.onCreate()` 中调用 `requestRebind()` 强制系统立即绑定监听器
